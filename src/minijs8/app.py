...
class MiniJS8App:
    ...
    async def run(self) -> None:
        ...
        # Emergency-arm gesture. 3-second hold for arming the SOS
        # beacon; 3-second hold for disarming. The completion
    ...
    def _start_tx_pipeline_best_effort(self) -> None:
        ...
        # Recover from any prior daemon run. Two-step process:
        #
        # 1. PURGE stale broadcast-kind rows (SOS, HEARTBEAT, CQ,
        #    ALLCALL). These are time-critical operator-initiated
        #    broadcasts that must NEVER auto-resume after a restart.
        #    An SOS from an earlier session may now be wrong (situation
        #    changed, operator is fine, etc.); a stale CQ has no
        #    semantic meaning across a reboot; HEARTBEATs are
        #    regenerated by the beacon thread on its own schedule
        #    if the operator had HBMode enabled.
        #
        #    This step was added in May 2026 after a stale SOS from
        #    a previous deployment was resurrected on every boot,
        #    pinning both Pi Zero 2W cores at 100% as the encoder
        #    failed-and-retried on malformed wire text. The CPU
        #    starvation made the UI unresponsive — looked like a
        #    keyboard lockout but was really render-thread starvation.
        #
        # 2. RECOVER personal-message rows (DIRECTED, REPLY). These
        #    do have recoverable intent — "I was about to reply to
        #    K1ABC and got rebooted" is preserved across the restart.
        #    The encoded-audio cache is in-memory only, so any rows
        #    in QUEUED state have lost their cached audio; push them
        #    back to ENCODING so the worker re-renders. Also handles
        #    ENCODING rows that never finished.
        try:
